...
void LGCShQ::recalculateSlowStartThreshold()
{
        ...
        // FIXME TODO - Does this formula computes the amount of outstanding data?
...
void LGCShQ::receivedDataAck(uint32 firstSeqAcked)
{
        ...
        if (state->dupacks >= DUPTHRESH) { // DUPTHRESH = 3
                ...
                if(state->ece) {
                        state->dctcp_marked++;
                        if (simTime() >= conn->tcpMain->par("rec"))
                ...
                if (state->lgcc_pacing) {
                        ...
                        if ((now - state->ecnnum_lastCalcTime >= state->minrtt * 1)) {// state->minrtt * 1
                                TCPEventCode event = TCP_E_IGNORE;
                                processRateUpdateTimer(event);
                        }
                }
        ...
        if (state->sack_enabled && state->lossRecovery) {
                ...
                // in the network."
                        ...
                        // segments as follows:"
                        // Note: Typecast needed to avoid prohibited transmissions
...
{
        /* kr1stj0n: Skip lgcc_pacing for now */
        /* if (state->lgcc_pacing) */
        /*         TCPTahoeRenoFamily::processRateUpdateTimer(event); */

        /* kr1stj0n: Move this IF statement to lgc_init */
        if (state->ecnnum_cntr == 0) {
                state->ecnnum_maxRate = conn->tcpMain->par("ldatarate");

                double d = conn->tcpMain->par("alpha");
                if(d != 0.0)
                        state->ecnnum_alpha = d;

                d = conn->tcpMain->par("phi");
                if(d != 0.0)
                        state->ecnnum_phi = d;

                state->ecnnum_Rate = (state->snd_cwnd * 8 / (double)state->minrtt.dbl());

                if (rateVector && simTime() >= conn->tcpMain->par("rec"))
                        rateVector->record(state->ecnnum_Rate);

                double logP = conn->tcpMain->par("logP");
                double coef = conn->tcpMain->par("coef");

                state->ecnnum_cntr++;
        }

        simtime_t now1 = simTime();
        double F = state->dctcp_marked / state->dctcp_total;
        if (F >= 0.9)
                state->ecnnum_fraction = (1 - state->ecnnum_alpha) * state->ecnnum_fraction
                                         + state->ecnnum_alpha * F;
        else
        ...
        if (loadVector && simTime() >= conn->tcpMain->par("rec"))
                loadVector->record(state->dctcp_marked / state->dctcp_total);
        if (calcLoadVector && simTime() >= conn->tcpMain->par("rec"))
